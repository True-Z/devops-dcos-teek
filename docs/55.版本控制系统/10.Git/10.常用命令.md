...
| 场景         | 常用命令组合                   | 说明                     |
| ------------ | ------------------------------ | ------------------------ |
| 查看当前状态 | `git status -sb`               | 看分支、暂存、未暂存改动 |
| 提交代码     | `git add .` -> `git commit -m` | 最常用提交流程           |
| 查看历史     | `git log --oneline --graph`    | 快速看提交链路           |
| 临时存档改动 | `git stash` / `git stash pop`  | 切任务时最常用           |
| 拉取远端更新 | `git pull --rebase`            | 减少无意义 merge 提交    |

## 阅读建议

- 如果你只是日常开发，先掌握“高频场景速查”里的那几组命令。
- 只有在遇到特殊场景时，再继续向下查具体命令的 `SYNOPSIS` 和 `USE`。
- 这页像一本命令手册，核心作用是回查，而不是要求一次记住所有命令。
...
```sh
git rebase [master] # 将当前分支的基底改为 master 的最新提交
...
```sh
git pull origin master:<localBranch> # 拉取 origin 主机的 master 分支到本地的 localBranch 分支
git pull origin master # 省略本地分支名则默认分支名相同
...
- 仓库内相关： [Git 简介](/vcs/git-intro)、[Git 解决方案](/vcs/git-solution)
