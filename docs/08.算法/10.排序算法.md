...
| 场景                   | 优先考虑                       | 原因                       |
| ---------------------- | ------------------------------ | -------------------------- |
| 数据量小、实现简单     | 冒泡 / 插入 / 选择             | 容易理解，适合教学和小数据 |
| 通用高性能排序         | 快速排序 / 归并排序            | 综合性能更好               |
| 接近有序的数据         | 插入排序                       | 插入成本低，实际表现好     |
| 需要稳定排序           | 归并排序 / 插入排序 / 冒泡排序 | 相等元素相对顺序可保持     |
| 已知整数范围且范围不大 | 计数排序 / 桶排序 / 基数排序   | 可突破比较排序下界         |

## 最小使用路径

```text
先看数据规模和是否要求稳定 -> 再看是否是整数且范围明确 -> 最后选择比较排序或非比较排序
```

## 阅读建议

- 如果你当前只想知道“该用哪种排序”，优先看“选型速查”和每种排序前的说明。
- 如果你需要自己手写实现，再继续看后面的代码示例。
- 代码较长时，不必一口气看完全部，先抓住适用场景和核心思路。
...
- 仓库内相关： [算法简介](/algorithm-intro)、[算法](/algorithm)、[查找算法](/algorithm/search)
- 官方文档： [排序算法 - Wikipedia](https://zh.wikipedia.org/wiki/%E6%8E%92%E5%BA%8F%E7%AE%97%E6%B3%95)
